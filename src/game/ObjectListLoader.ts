        ...
        Object.values(objectListConf).forEach((olObject: any) => {
            ...
            if (entityType === EntityType.TV_CAMERA) {
                ...
                if (entityType === EntityType.TOOLSTATION) {
                    ...
                    raider.changeActivity()
                    raider.sceneEntity.createPickSphere(raider.stats.PickSphere, raider)
                    raider.addToScene(entity.primaryPathSurface.getCenterWorld2D(), radHeading - Math.PI / 2)
                    entityMgr.raiders.push(raider)
                    EventBus.publishEvent(new RaidersChangedEvent(entityMgr))
                    ...
                    smallHeli.changeActivity()
                    smallHeli.sceneEntity.createPickSphere(smallHeli.stats.PickSphere, smallHeli)
                    smallHeli.addToScene(entity.primaryPathSurface.getCenterWorld2D(), radHeading + Math.PI)
                    ...
                    walker.changeActivity()
                    walker.sceneEntity.createPickSphere(walker.stats.PickSphere, walker)
                    walker.addToScene(entity.primaryPathSurface.getCenterWorld2D(), radHeading + Math.PI)
                ...
                const smallDigger = VehicleFactory.createVehicleFromType(entityType, sceneMgr, entityMgr)

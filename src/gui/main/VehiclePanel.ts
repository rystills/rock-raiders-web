...
abstract class VehiclePanel extends DependencyCheckPanel {
    ...
    addVehicleMenuItem(itemKey: string, entityType: EntityType) {
        ...
        item.onClick = () => this.publishEvent(new RequestVehicleSpawn(entityType, 1))
        item.onClickSecondary = () => {
            if ((this.requestedVehiclesByType.get(entityType) || 0) > 0) {
                this.publishEvent(new RequestVehicleSpawn(entityType, -1))
            }
        }
